...
char steam_id[32] = ""; 
char api_token[64] = ""; // Steam Web API Key
...
// Expanded label and tracking arrays to include real-time presence and ban metrics
...
void drawHotspotInfo(const char* ssid) {
  ...
  // Changed to fit 128px screen width (max 21 chars per line)
  display.setCursor(0, 44);
  display.println("Connect to WiFi");      // 15 characters
  display.setCursor(0, 54);
  display.println("to setup device.");     // 16 characters
  display.display();
}
...
void drawDisplay(bool isConnected) {
  ...
  // Truncate extremely long strings on the big layout field
...
void handleWifiReset() {
  server.send(200, "text/html", "<html><body style='background:#1b2838;color:#fff;font-family:sans-serif;text-align:center;padding-top:50px;'><h2>Resetting Wi-Fi...</h2></body></html>");
  delay(1000);
  WiFiManager wm;
  wm.resetSettings();
  ESP.restart();
...
bool fetchAndParseProfile(const char* targetId, const char* apiKey) {
  ...
  if (httpCode == 200) {
    ...
    if (!error && doc["response"]["players"][0]) {
      ...
      // Index 6: Account Age Calculation
...
void setup() {
  ...
  // Attach callback to draw Hotspot info on OLED when AP mode triggers
  ...
  // Wait for time to be synced (max 10 seconds timeout)
  unsigned long ntpStart = millis();
  while (time(nullptr) == 0 && millis() - ntpStart < 10000) {
    delay(100);
  }
  
  if (time(nullptr) == 0) {
    Serial.println("[WARN] NTP sync timed out. Age calc may drift.");
  } else {
    Serial.println("[SYSTEM] Time synchronized successfully.");
  }

  // Once connected, start web server and proceed to main loop
