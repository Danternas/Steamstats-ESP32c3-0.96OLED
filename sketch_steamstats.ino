...
bool fetchAndParseProfile(const char* targetId, const char* apiKey) {
  ...
  if (httpCode == 200) {
    ...
    if (!error && doc["response"]["players"][0]) {
      ...
      // Index 6: Account Age Calculation (Years & Days)
      if (!player["timecreated"].isNull()) {
        ...
        if (currentTime > creationTime && currentTime > 1000000) {
            ...
            // Format as "Xy Yd" to fit OLED cleanly
  ...
  // --- 4. Library Details (Playtime & Games) ---
  ...
  if (httpCode == 200) {
    ...
    if (!error && !bigDoc["response"].isNull()) {
      ...
      // Index 2: Calculate Combined Lifetime Playtime (Days & Hours Only)
      if (!resp["games"].isNull()) {
        ...
        // Format as "Xd Xh" to strictly avoid OLED truncation
